...
(function() {
  ...
  function Signaler(root, socketURL, socketEvent) {
    ...
    root.startBroadcasting = function() {
      if (!root.MediaStreamTrack)
        throw 'Offerer must have media stream.';

      (function transmit() {
        socket.customSend({userid: root.userid, broadcasting: true});
        !self.participantFound && !self.stopBroadcasting && setTimeout(transmit, 3000);
      })();
    };

    root.sendParticipationRequest = function(userid) {
      socket.customSend({participationRequest: true, userid: root.userid, to: userid});
    };
    ...
    root.acceptRequest = function(userid) {
      root.peers[userid] = Offer.createOffer(merge(options, {MediaStreamTrack: root.MediaStreamTrack}));
    };
    ...
    var options = {
      onsdp: function(sdp) {
        socket.customSend({userid: root.userid, sdp: sdp, to: root.participant});
      },
      onicecandidate: function(candidate) {
        socket.customSend({userid: root.userid, candidate: candidate, to: root.participant});
      },
    ...
    function closePeerConnections() {
      ...
      if (root.MediaStreamTrack)
        root.MediaStreamTrack.getVideoTracks()[0].stop();
    ...
    root.close = function() {
      socket.customSend({userLeft: true, userid: root.userid, to: root.participant});
  ...
  var Offer = {
    createOffer: function(config) {
      ...
      if (config.MediaStreamTrack)
        peer.addStream(config.MediaStreamTrack);
    ...
    addIceCandidate: function(candidate) {
      this.peer.addIceCandidate(new RTCIceCandidate({sdpMLineIndex: candidate.sdpMLineIndex, candidate: candidate.candidate}));
    }
  };
